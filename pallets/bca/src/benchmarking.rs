...
benchmarks! {
    ...
    }: _(SystemOrigin::Signed(caller.clone()), Default::default(), Edition{proofs: u8::MAX, prints: u16::MAX})
    verify {
    ...
    create_print {
        let caller: T::AccountId = whitelisted_caller();
        Bca::<T>::create_collection(SystemOrigin::Signed(caller.clone()).into(), Default::default(), Edition{proofs: u8::MAX, prints: u16::MAX})?;
